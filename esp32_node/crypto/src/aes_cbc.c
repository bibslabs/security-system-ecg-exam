...
uint8_t * compress_chunk(uint8_t *input, uint32_t size){
	esp_aes_crypt_cbc( &ctx, ESP_AES_ENCRYPT, size, iv, (uint8_t*)plaintext, (uint8_t*)encrypted );
	return (uint8_t*)encrypted;
}
